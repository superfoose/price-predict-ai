...
API_KEY = 'PK91UI64MT4CHQNH3K5C'
API_SECRET = 'rDws897Ck5y42lyctQijrKjnXlRCDnthJ9jKi4qB'
...
def get_data(symbol):
    ...
    def yesterday_date_components():
        current_date = datetime.now()
        yesterday = current_date - timedelta(days=25)
        year = yesterday.year
...
def ml_ind(df):
    ...
    def generate_labels(df):
        ...
        df.loc[df["RSI_14"] > df["RSI_14"].shift(1), 'Trend'] = 1  # Bullish trend

        # df.loc[df['Close'] <= df['SMA_10'], 'Trend'] = -1  # Bearish trend
        df.loc[df["RSI_14"] < df["RSI_14"].shift(1), 'Trend'] = -1  # Bearish trend
